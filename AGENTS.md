...
- **Latest release:** `1.8.1` (Review cleanup — pop-out correctness via `activeDocument`/injected `Document` + `Node.instanceOf`, `builtin-modules`→`node:module`, no `!important`, `no-misused-promises`, build-provenance attestations, SECURITY disclosures. Addresses the Obsidian automated-review warnings; `lint:obsidian` 70→0. `minAppVersion` 1.5.7). Live on both remotes; GitHub release built with 3 assets + provenance attestation, CI green (verified).
...
- **Unreleased on `main`:** nothing pending. **1.8.1 is live on both remotes (GitHub release: 3 assets + provenance attestation, CI green).** The Obsidian community submission (1.8.0) passed the automated scan with no errors; 1.8.1 clears the advisory warnings. Mobile interaction verified on a real iPhone. Residual portal warnings left on purpose: a few `no-unsafe-*` + one `querySelector as HTMLElement` are portal-eslint artifacts (our `tsc` against real obsidian types is clean).
