...
- **Roadmap (next):** No external gate is left — the listing is live. **A11y is the only substantial open item**, and it is real: measured 2026-09-03, the tree has no live region at all (`aria-live`/`role=status`: 0 occurrences), no `aria-level`/`setsize`/`posinset` in `core/render.ts`, and no `aria-pressed`/`aria-current` on the mode pills — i.e. gap-audit §5.1–5.5 stand as written. 5.2 (live regions for parse/schema errors, match count, copy) and 5.3 (accessible names on the inline-edit, rename and search inputs) are the cheap half. Then the 2.x feature ideas (§6: schema autocompletion, multi-select, `.jsonl`; §3.3–3.13); older open: cross-container drag-drop, `$schema` URL fetching, real pointer-events touch-drag.
