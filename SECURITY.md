...
- **Companion JSON Schema as a ReDoS vector.** A `*.schema.json` dropped next to a data file is compiled with @cfworker/json-schema; a schema author controls regex `pattern` / `patternProperties` values, and a catastrophic-backtracking regex against attacker-influenced data can hang the UI thread. Mitigations (1.5.0):
  ...
  - **Cheap pre-checks** before @cfworker/json-schema compiles: oversized schemas (> ~1 MB) are rejected, and a nested-quantifier heuristic rejects the classic catastrophic shapes (`(a+)+`, `(x+)*`, `(.*)+`, `(a{1,}){1,}`, …).
...
- **No dynamic code generation.** The validator is @cfworker/json-schema, an eval-free tree-walking interpreter: it evaluates a schema by traversing it directly, without `new Function` or `eval`. (The previous Ajv-based build compiled validators with `new Function`, which is why an automated scan reported "dynamic code execution"; that disclosure no longer applies to this plugin.) **No plugin code uses `eval` or `new Function`.**
...
Issues outside this surface (e.g. supply-chain attacks on the bundled `@cfworker/json-schema` or the Obsidian-provided `@codemirror/*` packages) should be reported to the upstream maintainers; I will pull in fixes as they land.
